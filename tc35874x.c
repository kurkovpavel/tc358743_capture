				     ...
				     struct v4l2_dv_timings *timings)
{
	...
	if (width == 1920 && height == 1080) {
		/* Assume 148.5 MHz for 1080p */
		bt->pixelclock = 148500000;
	} else if (width == 1280 && height == 720) {
		bt->pixelclock = 74250000;
	} else {
		/* Fallback to calculation but fix the fps */
		unsigned int actual_fps = 60; /* Default to 60 */
		bt->pixelclock = frame_width * frame_height * actual_fps;
	}
...
/* Force 1080p60 resolution - bypass detection */
static void tc35874x_force_1080p60(struct v4l2_subdev *sd)
{
    struct tc35874x_state *state = to_state(sd);
    struct v4l2_dv_timings timings;
    
    v4l2_info(sd, "FORCING 1920x1080@60 with standard CEA-861 timings\n");
    
    /* Use standard CEA-861 1080p60 timings */
    ...
    /* Standard 1080p60 timings (total: 2200x1125) */
    timings.bt.pixelclock = 148500000;  /* 148.5 MHz for 1080p60 */
    ...
    timings.bt.standards = V4L2_DV_BT_STD_CEA861;
    timings.bt.flags = V4L2_DV_FL_HALF_LINE;
    ...
    v4l2_info(sd, "1080p60 forced successfully - total: 2200x1125, pixelclock: %llu Hz\n",
...
static void tc35874x_format_change(struct v4l2_subdev *sd)
{
    ...
    int old_fps = 0, new_fps = 0;
    u8 sys_status, vi_status1, vi_status3;
    bool need_correction = false;
    
    /* Read hardware status registers */
    sys_status = i2c_rd8(sd, SYS_STATUS);
    vi_status1 = i2c_rd8(sd, VI_STATUS1);
    vi_status3 = i2c_rd8(sd, VI_STATUS3);
    
    v4l2_info(sd, "=== TC35874x FORMAT CHANGE DEBUG ===\n");
    v4l2_info(sd, "SYS_STATUS: 0x%02x (5V=%d, TMDS=%d, SYNC=%d, HDMI=%d)\n",
              sys_status,
              !!(sys_status & MASK_S_DDC5V),
              !!(sys_status & MASK_S_TMDS),
              !!(sys_status & MASK_S_SYNC),
              !!(sys_status & MASK_S_HDMI));
    v4l2_info(sd, "VI_STATUS1: 0x%02x (Interlaced=%d, DeepColor=%d)\n",
              vi_status1,
              !!(vi_status1 & MASK_S_V_INTERLACE),
              (vi_status1 >> 2) & 0x03);
    v4l2_info(sd, "VI_STATUS3: 0x%02x (ColorSpace=%d, Limited=%d)\n",
              vi_status3,
              (vi_status3 >> 1) & 0x0F,
              !!(vi_status3 & MASK_LIMITED));

    if (tc35874x_get_detected_timings(sd, &timings)) {
        v4l2_info(sd, "No signal detected!\n");
        enable_stream(sd, false);
        if (sd->devnode)
            v4l2_subdev_notify_event(sd, &tc35874x_ev_fmt);
        v4l2_info(sd, "=== END DEBUG ===\n");
        return;
    }
    
    /* Print detected timings in detail */
    v4l2_info(sd, "Detected timings:\n");
    v4l2_info(sd, "  Active: %dx%d\n", timings.bt.width, timings.bt.height);
    v4l2_info(sd, "  Total: %dx%d\n", 
              V4L2_DV_BT_FRAME_WIDTH(&timings.bt),
              V4L2_DV_BT_FRAME_HEIGHT(&timings.bt));
    v4l2_info(sd, "  Front porch: H=%d, V=%d\n", 
              timings.bt.hfrontporch, timings.bt.vfrontporch);
    v4l2_info(sd, "  Sync: H=%d, V=%d\n", 
              timings.bt.hsync, timings.bt.vsync);
    v4l2_info(sd, "  Back porch: H=%d, V=%d\n", 
              timings.bt.hbackporch, timings.bt.vbackporch);
    v4l2_info(sd, "  Pixel clock: %llu Hz (%llu MHz)\n", 
              (unsigned long long)timings.bt.pixelclock, 
              (unsigned long long)(timings.bt.pixelclock / 1000000));
    v4l2_info(sd, "  Interlaced: %s\n", 
              timings.bt.interlaced ? "Yes" : "No");
    
    /* FIX: Correct misdetected timings for 1080p using pixel clock */
    if (timings.bt.width == 1920 && timings.bt.height == 1080 &&
        timings.bt.hsync == 280 && timings.bt.hfrontporch == 0 &&
        timings.bt.vsync == 45 && timings.bt.vfrontporch == 0) {
        
        /* Use pixel clock to determine actual FPS */
        if (timings.bt.pixelclock >= 148000000 && timings.bt.pixelclock <= 149000000) {
            v4l2_info(sd, "*** CORRECTING 1080p60 TIMINGS ***\n");
            /* Keep 148.5MHz pixel clock, just fix sync values */
            timings.bt.pixelclock = 148500000;
            timings.bt.hfrontporch = 88;
            timings.bt.hsync = 44;
            timings.bt.hbackporch = 148;
            timings.bt.vfrontporch = 4;
            ...
            timings.bt.standards = V4L2_DV_BT_STD_CEA861;
            timings.bt.flags = V4L2_DV_FL_HALF_LINE;
            v4l2_info(sd, "Applied 60fps timings (pixelclock: 148.5 MHz)\n");
            need_correction = true;
        } 
        else if (timings.bt.pixelclock >= 74000000 && timings.bt.pixelclock <= 75000000) {
            v4l2_info(sd, "*** CORRECTING 1080p30 TIMINGS ***\n");
            ...
            timings.bt.standards = V4L2_DV_BT_STD_CEA861;
            timings.bt.flags = V4L2_DV_FL_HALF_LINE;
            v4l2_info(sd, "Applied 30fps timings (pixelclock: 74.25 MHz)\n");
            need_correction = true;
        }
    }
    
    /* Safety check: ensure state has valid timings */
    if (state->timings.bt.pixelclock > 0) {
        old_fps = fps(&state->timings.bt);
        new_fps = fps(&timings.bt);
        v4l2_info(sd, "Old FPS: %d, New FPS: %d\n", old_fps, new_fps);
    } else {
        v4l2_info(sd, "State timings not initialized yet\n");
    }
    
    /* Check if timings changed OR FPS changed OR we corrected */
    if (need_correction ||
        state->timings.bt.pixelclock == 0 ||
        !v4l2_match_dv_timings(&state->timings, &timings, 0, false) || 
        old_fps != new_fps) {
        
        v4l2_info(sd, ">>> FORMAT CHANGE DETECTED! <<<\n");
        v4l2_info(sd, "Change: %dx%d@%d -> %dx%d@%d\n",
                  state->timings.bt.width, state->timings.bt.height, old_fps,
                  timings.bt.width, timings.bt.height, new_fps);
        
        enable_stream(sd, false);
        tc35874x_s_dv_timings(sd, &timings);
        
        /* Also update state timings */
        state->timings = timings;
        
        v4l2_info(sd, "New timings applied successfully\n");
    } else {
        v4l2_info(sd, "No significant change detected\n");
    }
    
    v4l2_print_dv_timings(sd->name, "Current format: ", &timings, true);
